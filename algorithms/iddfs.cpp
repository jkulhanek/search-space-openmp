...
void iddfs_run_rec(std::shared_ptr<const state> node, unsigned int treshold, std::shared_ptr<const state> & result);
...
void iddfs_run_rec(std::shared_ptr<const state> node, unsigned int treshold, std::shared_ptr<const state> & result){
    ...
    for(int i = 0; i < children.size(); ++i){
        ...
        #pragma omp task shared(result) if(treshold > 3)
        iddfs_run_rec(child, treshold - 1, result);
    }
}
...
std::shared_ptr<const state> iddfs(std::shared_ptr<const state> root) {
    ...
    while(result == nullptr){
        #pragma omp parallel
        {
            #pragma omp single
            {
                iddfs_run_rec(root, treshold, result);                
            }
       ...
       /* // do the level search
        treshold = next_treshold;
        result = iddfs_run_iter(root, treshold, next_treshold);
        next_treshold = max(next_treshold, treshold + 1);

        cout << "iteration done " << i << "\n";
        i++;*/
        // cout << "iteration done " << i++ << "\n";
